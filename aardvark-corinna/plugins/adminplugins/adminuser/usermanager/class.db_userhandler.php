...
class UserDBHandler implements iDbHandler {
	...
	public function dbFetchUserMatrix() {
		$sql = $this->safesql->query("SELECT u.user_id, u.user_name, u.user_pass, p.perm_name, u.user_created, u.user_last_login, u.user_login_tries " .
		...
		while ($row = $this->dbo->getRows($sql)) {
			...
			if ($row['user_last_login'] == "0000-00-00") {
				$login = "-";
			} else {
				$login = $row['user_last_login'];
			}
			$user[$i] = array(
				'id' 	=> $row['user_id'],
				'name'	=> $row['user_name'],
	...
	public function dbFetchUserInfo($userid) {
		$sql = $this->safesql->query("SELECT u.user_id, u.user_name, u.user_pass,  p.perm_name, u.user_created, u.user_last_login, u.user_login_tries " .
				"FROM %s AS u LEFT JOIN %s AS p ON u.user_perm=p.perm_id WHERE u.user_id=%i", 
			array('pommomod_user', 'pommomod_perm', $userid) );
		while ($row = $this->dbo->getRows($sql)) {
			
			//TODO if lastlogin VOR created -> KOMISCH?????
			if ($row['user_last_login'] == "0000-00-00 00:00:00") {	//TIMESTAMP, not DATETIME
				$login = "-";
			} else {
				$login = $row['user_last_login'];
			}
	...
	public function dbAddUser($user, $pass, $perm) {
		if ($this->dbCheckUserName($user) == 0) {
			...
			$sql = $this->safesql->query("INSERT INTO %s (user_name, user_pass, user_perm, user_created, user_last_login, user_login_tries ) " .
	...
	public function dbEditUser($id, $user, $pass, $perm) {
		...
		$sql = $this->safesql->query("UPDATE %s SET user_name='%s', user_pass='%s', user_perm='%s' 
				WHERE user_id=%i",
			array('pommomod_user', $user, $pass, $perm, $id ) );

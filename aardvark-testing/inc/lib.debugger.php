...
function bmBacktrace() {
	...
	************************************************/
	...
	class Print_a_class {
		...
		function _handle_whitespace($string) {
			...
			$string = preg_replace(array (
				'/&nbsp;$/',
				'/^&nbsp;/'
			), '<span style="color:red;">_</span>', $string); /* replace spaces at the start/end of the STRING with red underscores */
		...
		function print_a($array, $iteration = FALSE, $key_bg_color = FALSE) {
			...
			foreach ($array as $key => $value) {
				...
				if ($only_numeric_keys && $this->limit && $this->limit == $i++)
					break; /* if print_a() was called with a fourth parameter #TODO# */
				...
				elseif ($type == 'object') {
					if ($this->show_object_vars) {
						...
						$this->print_a(array (
							'CLASS_NAME' => $objects_class
						), TRUE, '204FB8');
						$this->print_a(array (
							'CLASS_VARS' => get_class_vars($objects_class
						)), TRUE, '2066B8');
						$this->print_a(array (
							'CLASS_METHODS' => get_class_methods($objects_class
						)), TRUE, '2067EB8');
						$this->print_a(array (
	...
	function script_runtime() {
		...
		$MICROTIME_START	= explode(' ', $MICROTIME_START);
	...
	function show_vars($show_all_vars = FALSE, $show_object_vars = FALSE, $limit = 5) {
		...
		$vars_arr['_POST'] = array (
			'$_POST',
			'#F49AC1'
		);
		$vars_arr['_FILES'] = array (
			'$_FILES',
			'#82CA9C'
		);
		$vars_arr['_SESSION'] = array (
			'$_SESSION',
			'#FCDB26'
		);
		$vars_arr['_COOKIE'] = array (
			'$_COOKIE',
			'#A67C52'
		);

		if ($show_all_vars) {
			$vars_arr['_SERVER'] = array (
				'SERVER',
				'#A186BE'
			);
			$vars_arr['_ENV'] = array (

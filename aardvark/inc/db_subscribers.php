...
function & dbGetSubscriber(& $dbo, & $input, $retunType = 'detailed', $table = 'subscribers') {
	...
	if (empty ($dbMatch)) {
		die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbGetSubscriber() -> Bad Input Passed.');
	}
	switch ($retunType) {
		case 'id' :
	...
	die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbGetSubscriber() -> Reached end. Bad type sent?.');
}
	...
	switch ($type) {
		...
		case 'password' :
			...
			if (empty ($pending_id) || !is_numeric($pending_id))
				die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbAddPending() -> Unable to fetch pending_id. Notify Administrator.');
			...
			break;
		default:
			die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbAddPending() -> Unknown type passed.');
	}
...
function dbPendingDel(& $dbo, $input = NULL) {
	...
	if (is_numeric($input[0])) // do not allow IDs to be passed to pendingDel...
		die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbPendingDel() -> Bad Input Passed.');
...
function dbSubscriberAdd(& $dbo, & $arg, $pending = FALSE) {
	...
	if (!is_array($subscriber) || empty ($subscriber['email']) || !isset ($subscriber['data']))
		die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbSubscriberAdd() -> Subscribers array not sane. Notify Administrator.');
	...
	if (empty ($subscriber_id) || !is_numeric($subscriber_id))
		die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">dbSubscriberAdd() -> Unable to fetch subscriber_id. Notify Administrator.');

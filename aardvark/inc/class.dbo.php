...
class dbo {
	...
	function dbo($username = NULL, $password = NULL, $database = NULL, $hostname = NULL, $tablePrefix = NULL) {
		...
		if (!$this->_link)
			die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">Could not establish database connection. Verify your config.php settings in the setup directory. <br><br>&nbsp;&nbsp;See the <a href="'.bm_baseUrl.'/docs/readme.html">README</a> file for help.');

		if (!@ mysql_select_db($database, $this->_link))
			die('<img src="'.bm_baseUrl.'/img/icons/alert.png" align="middle">Connected to database server but could not select database: "'.$database.'". <br><br>&nbsp;&nbsp; Does this database exist? Verify your config.php settings in the setup directory.');
	}
